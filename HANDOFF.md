...
### 2-1. 시간 변환 규칙 (Sprint 1)
인하대점 `근무시간` 시트의 **수기 월력 이미지 3장**을 판독해 역설계함. 규칙:
> **근무시간 = 퇴근 − 출근** (퇴근≤출근이면 +24h), **30분 단위**, **휴게 차감 없음**.
- 이현진 일별 합계 = 158h, 엑셀 셀 T3=158과 일치. 6개 시간대 사례 전부 일치.
- 앱 로직(`pairEvents` 등) Node 유닛테스트 8개 통과(자정넘김·미완결·고아·분할근무 합산).
- **주의**: 6~7시간 연속근무에 휴게 미차감 = 현재 매장 관행. 법정 휴게 관점 미검증(회계사 확인 대상).
