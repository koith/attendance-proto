...
(()=>{
  ...
  function style(){if(document.getElementById('storeControlsV1Style'))return;const s=document.createElement('style');s.id='storeControlsV1Style';s.textContent=`
    ...
    .store-time-pair{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);gap:5px;align-items:center}.store-time-pair span{text-align:center;color:var(--text-muted);font-weight:700}.store-time-pair input{text-align:center;min-width:0}.store-close-hint{font-size:.7rem;color:var(--text-muted);margin-top:8px;line-height:1.45}.store-force-status{font-size:.72rem;color:var(--text-muted);margin-top:8px;min-height:1.1em}
  ...
  async function mount(){
    ...
    card.innerHTML=`<h3>매장 정보</h3><div class="store-settings-grid"><div><label>오픈 시간</label><div class="store-time-pair"><input id="storeOpenH" inputmode="numeric" pattern="[0-9]*" placeholder="07" aria-label="오픈 시"><span>:</span><input id="storeOpenM" inputmode="numeric" pattern="[0-9]*" placeholder="00" aria-label="오픈 분"></div></div><div><label>마감 시간 (24시 이후 가능)</label><div class="store-time-pair"><input id="storeCloseH" inputmode="numeric" pattern="[0-9]*" placeholder="25" aria-label="마감 시"><span>:</span><input id="storeCloseM" inputmode="numeric" pattern="[0-9]*" placeholder="00" aria-label="마감 분"></div></div><div><label>마감 후 유예 (분)</label><input id="storeGrace" inputmode="numeric" type="number" min="0" max="360" step="10"></div><div style="display:flex;align-items:end"><button class="btn btn-primary btn-block" id="storeSave">저장</button></div></div><div class="store-close-hint" id="storeCutoffHint"></div><div class="store-force-status" id="storeForceStatus"></div>`;
    before.parentNode.insertBefore(card,before);
    try{
      const s=await BE.storeSettingsGet();
      document.getElementById('storeOpenH').value=p(Math.floor(s.open_minute/60));
      document.getElementById('storeOpenM').value=p(s.open_minute%60);
    ...
    document.getElementById('storeSave').onclick=async()=>{
      ...
      const o=parseOpen(document.getElementById('storeOpenH').value,document.getElementById('storeOpenM').value),c=parseClose(document.getElementById('storeCloseH').value,document.getElementById('storeCloseM').value),g=Number(document.getElementById('storeGrace').value);
