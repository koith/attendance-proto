...
t('core already treats end-before-start as next day',()=>a(core.includes('if(n<0)n+=1440'),'overnight minute calculation missing'));
...
t('stored workday payload remains start/end only',()=>a(core.includes('out.push({weekday,start:x.start,end:x.end})'),'workday payload semantics changed'));
