...
computeMonthPayroll=async function(ym){const R=await base(ym);let tg=0,tn=0;for(const row of R.rows||[]){const c=row.contract,p=row.pay;if(p&&c?.night_allowance_enabled){const h=overlap(row.sessions,c.night_allowance_start||'22:00',c.night_allowance_end||'06:00')/3600,v=Number(c.night_allowance_value||0);const add=c.night_allowance_mode==='FLAT'?Math.round(h*v):Math.round(h*Number(p.wage||0)*v/100);p.nightHours=h;p.night=add;p.gross+=add;p.net=xrounddown(p.gross*(1-p.rate),-1)}if(p){tg+=p.gross;tn+=p.net}}R.totalGross=tg;R.totalNet=tn;return R};
})();