...
t('canonical daily/monthly schedule entries are actual results, not planners',()=>{a(dayEntry.includes('actual_attendance.html?view=day&v='),'daily entry still planner/stale');a(monthEntry.includes('actual_attendance.html?view=month&v='),'monthly entry still planner/stale');a(entryMode.includes("mode==='day'"),'daily result entry mode missing')});
t('planned editors are retained internally but not linked from actual attendance',()=>{a(plannedDay.includes('계획 스케줄 편집'),'planned daily file missing');a(plannedMonth.includes('계획 스케줄 편집'),'planned monthly file missing');a(!entryMode.includes('planned_daily_schedule.html'),'planned daily shortcut still exposed');a(!entryMode.includes('planned_monthly_schedule.html'),'planned monthly shortcut still exposed')});
t('mobile runtime is cache-busted and wired',()=>{a(html.includes('actual_attendance.css?v='),'css cache-bust missing');a(html.includes('actual_attendance.js?v='),'js cache-bust missing');a(html.includes('supabase_session_resilience_v1.js?v='),'session layer missing');a(js.includes("mode==='day'"),'entry mode missing from canonical runtime');a(!html.includes('actual_attendance_entry_mode_v1.js?v='),'legacy entry-mode patch must not load separately');a(css.includes('.calendar'),'calendar style missing');a(css.includes('.track'),'timeline style missing')});
