...
function renderMonth(){S.selectedDay=null;const [y,m]=S.ym.split('-').map(Number),days=new Date(y,m,0).getDate(),offset=new Date(y,m-1,1).getDay(),today=dayKey(kstToday());let html='<div class="weekhead"><div>일</div><div>월</div><div>화</div><div>수</div><div>목</div><div>금</div><div>토</div></div><div class="calendar">';for(let i=0;i<offset;i++)html+='<div class="day blank"></div>';for(let d=1;d<=days;d++){const day=`${S.ym}-${p2(d)}`,ss=sessionsForDay(day),issues=ss.filter(s=>isIssue(s,day)),people=new Set(ss.map(s=>Number(s.employee_id))).size,workedSec=ss.filter(s=>s.status==='COMPLETE').reduce((n,s)=>n+(s.sec||0),0),summary=ss.length?`<div class="day-summary"><b>${people}명</b><span>${dur(workedSec)}</span></div>`:'',warning=issues.length?`<div class="day-warning">확인 ${issues.length}</div>`:'';html+=`<button class="day${day===today?' today':''}${issues.length?' issue':''}" data-day="${day}"><span class="num">${d}</span><div class="day-overview">${summary}${warning}</div></button>`}html+='</div><div class="legend"><span><i class="dot"></i>근무 인원·완료시간</span><span><i class="dot warn"></i>확인 필요</span><span>날짜를 누르면 상세 타임라인</span></div>';el('app').innerHTML=html;document.querySelectorAll('[data-day]').forEach(b=>b.onclick=()=>renderDay(b.dataset.day))}
